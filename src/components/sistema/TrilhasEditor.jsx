        ...
        return (
          <div key={t.id} className="rounded-xl border border-purple-900/60 bg-slate-900/40 p-3 space-y-2.5">
            ...
            <div className="space-y-1">
              <p className="text-purple-400 text-xs">Tipos de marca (severidade maior sobrescreve menor)</p>
              {(t.tipos_marca || []).map((tm, k) => (
                <div key={tm.id} className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <label className="text-accent-300 text-xs flex items-center gap-1">símbolo
                  ...
                  <label className="text-accent-300 text-xs flex items-center gap-1">severidade
              ...
              <button onClick={() => setTrilha(i, { tipos_marca: [...(t.tipos_marca || []), { id: novoId('tm'), nome: '', simbolo: '', severidade: (t.tipos_marca?.length || 0) + 1 }] })}
                className="text-xs px-2 py-0.5 rounded-lg border border-dashed border-purple-700 text-purple-300 hover:text-white hover:border-purple-500 transition-colors">+ tipo</button>
            </div>

            {/* Comportamento */}
            <div className="flex items-center gap-3 flex-wrap">
              <label className="text-purple-300 text-xs flex items-center gap-1">trilha cheia:
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-purple-400 text-xs">cheia do tipo mais severo:</span>
              ...
              <div className="space-y-1 border-t border-purple-900/50 pt-2">
                ...
                {descansos.map(d => (
                  <div key={d.id} className="flex items-center gap-2 flex-wrap">
                    <span className="text-accent-300 text-xs w-24 truncate" title={d.nome}>{d.nome}:</span>
                    {(t.tipos_marca || []).map(tm => {
                      const regra = (rec[d.id] || {})[tm.id] || { modo: 'nada' }
                      return (
                        <label key={tm.id} className="text-purple-400 text-xs flex items-center gap-1">
                          {tm.nome || tm.simbolo}
