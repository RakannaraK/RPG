  ...
  return (
    <div className="bg-raised border border-border rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-border">
        <p className="text-ink text-sm font-semibold">Combate</p>
      </div>
      <div className="grid grid-cols-2 gap-px bg-void/30 p-px">
        {campos.map(campo => {
          ...
          if (campo.tipo === 'calculado') {
            ...
            return (
              <div key={campo.id} className="bg-raised flex flex-col items-center justify-center py-3 px-2 gap-1 relative group/calc">
          ...
          return (
            <div
              key={campo.id}
              className="bg-raised flex flex-col items-center justify-center py-3 px-2 gap-1"
            >
              {isDono ? (
                <input
                  type="text"
                  ...
                  placeholder="—"
                  className="w-full text-center text-ink font-bold text-2xl leading-none bg-transparent border-b border-border focus:border-accent-400 focus:outline-none pb-0.5"
                />
              ) : (
                <p className="text-ink font-bold text-2xl leading-none">
                  {getValor(campo.id) || '—'}
                </p>
              )}
